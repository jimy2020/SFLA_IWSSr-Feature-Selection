function Frogs = ImproweFrog_WithIwssr(Initialfrogs)
global var;
    Frogs = struct('SelVar',{},'AvgAcc',{});
    for pop = 1:size(Initialfrogs,1)
        Set(1,:) = Initialfrogs{pop};
        x=Initialfrogs{pop,1};
        y=var.Score(x);
        [~,IX] = sort(y,'descend');
        Set = Set(IX);
        Frogs(pop,1) = Func(Set);
        clear Set
    end
