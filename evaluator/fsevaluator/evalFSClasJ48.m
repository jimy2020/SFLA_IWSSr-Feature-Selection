function [ AC ] = evalFSClasJ48( X, Y, trainIDX, testIDX, featIDX, numF)
...
for i = 1:length(numF)
    ...
    AC(i) = J48_CV_estimate(trX,trY,tsX,tsY,0);

    fprintf('total features: %5i,   select features: %5i,   J48 acc value: %.3f\n', size(trX,2), l, AC(i));
    if oversel && ~cal
        cal = 1;
    end
end